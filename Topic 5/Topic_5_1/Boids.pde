...
class Boid {
  ...
  // Wraparound
  void borders() {
    if (position.x <= r && velocity.x <0) velocity.x = -velocity.x*0.9;
    if (position.y <= r && velocity.y <0) velocity.y = -velocity.y*0.9;
    if (position.x >= width-r && velocity.x >0) velocity.x = -velocity.x*0.9;
    if (position.y >= height-r&& velocity.y >0) velocity.y = -velocity.y*0.9;
  }
